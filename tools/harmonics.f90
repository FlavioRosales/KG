program harmonics 
    ...
    open(1,file=trim(filename))
            ...
            y_lm(l,m,:,:) = compute_spherical_harmonic(l,m,theta(:,:),phi(:,:))
    ...
    y_analitc(0,0,:,:) = 0.5d0/dsqrt(pi)

    y_analitc(1,1,:,:)  = -dsqrt(3.0d0/(8.0d0*pi))*exp(i_unreal*phi)*dsin(theta)
    y_analitc(1,0,:,:)  = dsqrt(3.0d0/(4.0d0*pi))*dcos(theta)
    y_analitc(1,-1,:,:) = (-1.0d0)**1 * dconjg(y_analitc(1,1,:,:))
    
    y_analitc(2,2,:,:)  = dsqrt(15.0d0/(32.0d0*pi))*exp(2.0d0*i_unreal*phi)*dsin(theta)**2
    y_analitc(2,1,:,:)  = -dsqrt(15.0d0/(8.0d0*pi))*exp(i_unreal*phi)*dsin(theta)*dcos(theta)
    y_analitc(2,0,:,:)  = dsqrt(5.0d0/(16.0d0*pi))*(3.0d0*dcos(theta)**2 - 1.0d0)
    y_analitc(2,-1,:,:) = (-1.0d0)**1 * dconjg(y_analitc(2,1,:,:))
    y_analitc(2,-2,:,:) = (-1.0d0)**2 * dconjg(y_analitc(2,2,:,:))
